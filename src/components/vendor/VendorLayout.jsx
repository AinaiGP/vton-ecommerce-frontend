  ...
  const NAV = [
    {
      ...
      items: [
        ...
        { icon: ShoppingCart, label: t("vendor.orders"), to: "/vendor/orders" },
    ...
    },
    {
      ...
      items: [
        { icon: Star, label: t("vendor.reviews"), to: "/vendor/reviews" },
        { icon: Eye, label: t("vendor.analytics"), to: "/vendor/analytics" },
        { icon: RotateCcw, label: "Refund Requests", to: "/vendor/refunds" },
      ],
    },
    {
      ...
      >
        ...
        <div className={s.sidebarBottom}>
          ...
          >
            ...
            {!collapsed && (
              <span className={s.navLabel}>{t("profile.logout")}</span>
            )}
          </button>
          <button className={s.collapseBtn} onClick={onToggle}>
            <ChevronLeft
              size={15}
              className={[s.collapseIcon, collapsed ? s.flipped : ""].join(" ")}
              style={{
                transform: collapsed
                  ? lang === "ar"
                    ? "rotate(0deg)"
          ...
          {notifOpen && (
            <div className={s.notifPanel}>
              <div className={s.notifHead}>
                <h3 className={s.notifTitle}>
                  {t("profile.notifications")} {unread > 0 && `(${unread})`}
                </h3>
                <button
              ...
              <ul className={s.notifList}>
                {notifs.map((n) => (
                  <li
                    key={n.id}
                    className={[s.notifItem, n.unread ? s.unread : ""].join(
                      " ",
                    )}
                  >
              ...
              <ul className={s.profileMenuList}>
                ...
                <li>
                  <Link
                    ...
                    onClick={() => setProfileOpen(false)}
                  >
                    <Lock size={14} /> {t("profile.password")}
                  </Link>
                </li>
                <li>
                  <button
          ...
          {(pageTitle || breadcrumb) && (
            <div className={s.pageHead}>
              <div className={s.pageTitleGroup}>
                ...
                {pageSubtitle && (
                  <p className={s.pageSubtitle}>{pageSubtitle}</p>
                )}
              </div>
